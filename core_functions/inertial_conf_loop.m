...
% Rnbd_R0_Array - array of Rnbd/R0
...
% 
% Output arguments:
% Value_th- pulse duration or Rnbd/R0 at inertial conf. threshold
...
% preallocate array
L_tau     = length(tau_Array);     % length of the array of pulse duration
L_Rnbd_R0 = length(Rnbd_R0_Array); % length of the array of Rnbd/R0

assert(~((L_tau>1)&&(L_Rnbd_R0>1)));% throw error if array size of both pulse duration and Rnbd/R0 >1

if L_tau > 1
    nLoop = L_tau;     % Loop along pulse duration
    Loop_Array = tau_Array;
else
    nLoop = L_Rnbd_R0; % Loop along energy density Rnbd/R0
    Loop_Array = Rnbd_R0_Array;
end

Results_Matrix = zeros(6,nLoop); % Row 1 Rmax, Row 2 Tosc, Row 3 R2tau/R0, Row 4 Pbd, Row 5 Ubd, Row 6 tau_L or Rnbd/R0
...
for i=1:nLoop
    Loop = Loop_Array(i);
    if L_tau >1 % Loop along pulse duration
        tau_L = Loop;
        Rnbd = R0*Rnbd_R0_Array; % single-element array
    else        % Loop along energy density Rnbd/R0
        Rnbd = R0*Loop;
        tau_L = tau_Array;       % single-element array
    end

    % inital calculation assuming no condensation to find Rmax and tRmax
    % For studying bubble expansion, the value of Rnc is not important. Here we simply use Rnc=Rnbd. In reality, Rnc depends on bubble size as well as plasma energy density.
    ...
    if length(pks)==1 % to check if the min. is the first collapse
        ...
        [~,index_Rmin] = min(R(locs2(1):locs2(2)));% find the minima between the 1st peak and the 2nd peak
...
% store results
    ...
    Results_Matrix(6,i) = Loop;
end

%% find the threshold values
if max(Results_Matrix(3,:)) > Inertial_threshold 
    Value_th = interp1(Results_Matrix(3,:), Results_Matrix(6,:), Inertial_threshold,'spline'); % Inertial_threshold R/R0 = 2^(1/3) 
    Tosc_th  = interp1(Results_Matrix(6,:), Results_Matrix(2,:), Value_th,'spline'); 
    Rmax_th  = interp1(Results_Matrix(6,:), Results_Matrix(1,:), Value_th,'spline'); 

    % Plots
    figure()
    subplot(5,1,1)
    semilogx(Results_Matrix(6,:),Results_Matrix(3,:),'g',Value_th,Inertial_threshold,'o','LineWidth',1.5);
    ylabel('R|_{t=2tau}(xR_0)')
    subplot(5,1,2)
    semilogx(Results_Matrix(6,:),Results_Matrix(1,:)*1e6,'r',Value_th,Rmax_th*1e6,'o','LineWidth',1.5);
    ylabel('R_{max} (\mum)')
    subplot(5,1,3)
    ...
    ylabel('Velocity (m/s)')
    subplot(5,1,5)
    semilogx(Results_Matrix(6,:),Results_Matrix(2,:)*1e6,'b',Value_th,Tosc_th*1e6,'o','LineWidth',1.5);

    if L_tau >1 % Loop along pulse duration
    xlabel('Pulse duration (s)')
    else        % Loop along Rnbd/R0
        xlabel('R_{nbd}/R_0')
    end
